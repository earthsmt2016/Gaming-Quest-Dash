...
export async function applyIssueFix(data: { file: string; currentCode: string; proposedCode: string }): Promise<{ ok: boolean; error?: string; requiresRestart?: boolean; requiresReload?: boolean }> {
  ...
  return { ok: true, requiresRestart: Boolean(raw.requiresRestart), requiresReload: Boolean(raw.requiresReload) };
}
